...
export const typographyStyles = cva("fj:font-fuji-primary fj:m-0", {
  variants: {
    variant: {
      display:
      ...
      subtitle:
        "fj:text-[length:calc(var(--fuji-text-base)_-_1px)] fj:font-medium fj:text-fuji-foreground-muted",
      ...
      caption:
        "fj:text-[length:calc(var(--fuji-text-xs)_-_1px)] fj:font-normal fj:text-fuji-foreground-subtle",
    },
  },
  defaultVariants: { variant: "body" },
});

export type TypographyVariant = NonNullable<VariantProps<typeof typographyStyles>["variant"]>;

export const TYPOGRAPHY_DEFAULT_TAG: Record<TypographyVariant, keyof React.JSX.IntrinsicElements> = {
  display: "h1",
  heading: "h2",
  title: "h3",
  subtitle: "p",
